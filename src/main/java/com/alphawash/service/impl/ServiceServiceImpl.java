...
public class ServiceServiceImpl implements ServiceService {
	...
	if (ObjectUtils.isNull(request)) {
    	...
    	BasicServiceResponse serviceResponse = serviceRepository.getBasicServiceByServiceCode(request.serviceCode())
				.orElseThrow(() -> new BusinessException(HttpStatus.BAD_REQUEST, "Service not found: " + request.serviceCode()));
	ServiceCatalog serviceCatalog = serviceCatalogRepository.findByCode(serviceResponse.getServiceCatalogCode())
				.orElseThrow(() -> new BusinessException(HttpStatus.BAD_REQUEST, "Error when tried to get service catalog: " + serviceResponse.getServiceCatalogCode()));
	Service service = serviceRepository.findByCode(request.serviceCode())
				.orElseThrow(() -> new BusinessException(HttpStatus.BAD_REQUEST, "Error when trying to get service: " + request.serviceCode()));
